...
# Allowed file types - NOW INCLUDES PDF, DOCX, TXT
ALLOWED_EXTENSIONS = {
...
def analyze_pdf(file_path):
    ...
    try:
        with open(file_path, 'rb') as file:
            ...
            if metadata:
                findings.append(f"PDF metadata found")
...
def analyze_docx(file_path):
    ...
    try:
        ...
        full_text = ' '.join([p.text for p in doc.paragraphs[:50]])  # First 50 paragraphs
...
def analyze_file_forensic(file_path):
    ...
    for pattern in suspicious_patterns:
        if pattern in filename:
            score += 10
            findings.append(f"⚠️Suspicious filename: '{pattern}' detected")
            break
